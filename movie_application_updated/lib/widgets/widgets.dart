...
class HomeScreenWidgets {
  TVShowsListDisp tvShowsListDisp = const TVShowsListDisp();
  MoviesListDisp moviesListDisp = const MoviesListDisp();
  ...
  Widget watchListCard({int? index, bool movie = true}) {
    final WatchlistDispController watchlistDispController = Get.find();
    return InkWell(
        onTap: () {
          Get.to(DetailsPage(
            index: movie
                ? int.parse(watchlistDispController
                        .movieWatchList.value[int.parse(index.toString())].rank
                        .toString()) -
                    1
                : int.parse(watchlistDispController
                        .tvShowWatchList.value[int.parse(index.toString())].rank
                        .toString()) -
                    1,
            movie: movie,
          ));
                      ...
                      decoration: BoxDecoration(
                          image: DecorationImage(
                              ...
                              image: NetworkImage(movie
                                  ? watchlistDispController.movieWatchList
                                      .value[int.parse(index.toString())].image
                                      .toString()
                                  : watchlistDispController.tvShowWatchList
                                      .value[int.parse(index.toString())].image
                                      .toString()))),
                    )),
                Container(
  ...
  Widget favIcon({bool mov = true, int? index}) {
    if (mov) {
      return listDispController2.movieList.value[int.parse(index.toString())]
                  .watchlist.value ==
              1
          ? const Icon(Icons.favorite)
          : const Icon(Icons.favorite_border);
    } else {
      return listDispController1.tvShowList.value[int.parse(index.toString())]
                  .watchlist.value ==
              1
          ? const Icon(Icons.favorite)
          : const Icon(Icons.favorite_border);
